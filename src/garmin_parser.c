...
// DEVICE_INFO
DECLARE_FIELD(DEVICE_INFO, device_index, UINT8) { garmin->cache.device_index = (UINT8)data; }
DECLARE_FIELD(DEVICE_INFO, product, UINT16) {
	// we're only interested in the values for the 'creator', i.e. the main device
	if (garmin->cache.device_index == 0)
		garmin->cache.product = (UINT16)data;
}
DECLARE_FIELD(DEVICE_INFO, serial_nr, UINT32Z) {
	// we're only interested in the values for the 'creator', i.e. the main device
	if (garmin->cache.device_index == 0)
		garmin->cache.serial_nr = (UINT32Z)data;
}
DECLARE_FIELD(DEVICE_INFO, firmware, UINT16) {
	// we're only interested in the values for the 'creator', i.e. the main device
	if (garmin->cache.device_index == 0)
		garmin->cache.firmware = (UINT16)data;
}
